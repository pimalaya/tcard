...
- **Parameter casing on a retyped line**: a line whose `TYPE` the document changed is rewritten with the document's spelling (lowercase) and its type parameters gathered into one `TYPE=`. A line whose types are unchanged, however they are spelled (`;WORK`, `TYPE="work,voice"`, one parameter per value), keeps its bytes.
